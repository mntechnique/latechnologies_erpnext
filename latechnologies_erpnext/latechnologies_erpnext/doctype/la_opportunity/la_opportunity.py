...
class LAOpportunity(TransactionBase):
	...
	def validate(self):
		self._prev = frappe._dict({
			"contact_date": frappe.db.get_value("LA Opportunity", self.name, "contact_date") if \
				(not cint(self.get("__islocal"))) else None,
			"contact_by": frappe.db.get_value("LA Opportunity", self.name, "contact_by") if \
				(not cint(self.get("__islocal"))) else None,
		})
	...
	def add_calendar_event(self, opts=None, force=False):
		...
		super(LAOpportunity, self).add_calendar_event(opts, force)
...
def make_quotation(source_name, target_doc=None):
	...
	doclist = get_mapped_doc("LA Opportunity", source_name, {
		"LA Opportunity": {
			"doctype": "Quotation",
			"field_map": {
...
def set_multiple_status(names, status):
	...
	for name in names:
		opp = frappe.get_doc("LA Opportunity", name)
		opp.status = status
		opp.save()
...
def auto_close_opportunity():
	...
	opportunities = frappe.db.sql(""" select name from `tabLA Opportunity` where status='Replied' and
		modified<DATE_SUB(CURDATE(), INTERVAL %s DAY) """, (auto_close_after_days), as_dict=True)

	for opportunity in opportunities:
		doc = frappe.get_doc("LA Opportunity", opportunity.get("name"))
		doc.status = "Closed"
...
@frappe.whitelist()
def make_crm_opportunity(source_name, target_doc=None):
	doclist = get_mapped_doc("LA Opportunity", source_name, {
		"LA Opportunity": {
			"doctype": "Opportunity",
		},
